...
function chart(points, synthetic=true) {
  ...
  const ns='http://www.w3.org/2000/svg'; const svg=document.createElementNS(ns,'svg'); svg.setAttribute('viewBox','0 0 1000 200'); svg.setAttribute('role','img'); svg.setAttribute('aria-label',synthetic ? 'Equity sintética reproducida' : 'Equity de replay importado sin validar');
  ...
  const label=document.createElementNS(ns,'text'); label.setAttribute('x','20'); label.setAttribute('y','18'); label.setAttribute('fill','#81968b'); label.setAttribute('font-size','11'); label.textContent=`${synthetic ? 'SINTÉTICO' : 'IMPORTADO · SIN VALIDAR'} · ${money(lo)} — ${money(hi)}`; svg.append(label); box.append(svg);
}
function render(s) {
  text('external-mutations',s.external_mutations); text('demo-checks',`${s.etoro_demo_read} / ${s.etoro_demo_write}`); text('close-check',s.close_reconciliation);
  ...
  rows('positions',s.positions || [],[x=>x.symbol,x=>x.units,x=>x.observed_units,x=>x.accounting_complete ? 'Completa' : 'PENDIENTE',x=>x.entry_price,x=>x.stop_price],'Sin exposición abierta');
  ...
  if(s.report){text('provenance',s.report.label); text('report-json',JSON.stringify(s.report,null,2)); chart(s.chart || [],s.report.data_manifest.synthetic); const metrics=$('report-metrics'); metrics.replaceChildren(); for(const [key,value] of Object.entries(s.summary_metrics || {})){const item=document.createElement('div'); item.textContent=key; const b=document.createElement('b'); b.textContent=value===null ? 'N/A' : String(value); item.append(b); metrics.append(item);}}
}
